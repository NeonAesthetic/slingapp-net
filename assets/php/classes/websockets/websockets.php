...
//            echo $message_object['action'] . "\n";
            switch($message_object["action"]) {
                ...
                {
                    $this->on_client_alter_video($user, $message, $room, $account);
                }break;
    ...
    abstract protected function on_client_chat($user_socket, $message, Room &$room, Account &$account);
    ...
    abstract protected function on_client_alter_voice($user_socket, $message, Room &$room, Account &$account);

    abstract protected function on_client_alter_video($user_socket, $message, Room &$room, Account &$account);
...
//                        msg("SKT: Client disconnected. TCP connection lost: ".$socket,WRN,CONT);
                        ...
                        if (!$user->handshake) {
                            ...
                            if (strpos($tmp, NL . NL) === false) {
                                continue;
                            } # when client has not finished sending header, wait before sending upgrade response
