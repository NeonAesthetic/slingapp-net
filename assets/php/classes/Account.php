...
class Account extends DatabaseObject
{
    ...
    public function addParticipant($roomID, $screenName)
    {
        $sql = "INSERT INTO Participants
                (AccountID, RoomID, ScreenName, Active)
                VALUES (:accountID, :roomID, :screenName, :active)";
        $statement = Database::connect()->prepare($sql);

        if ($statement->execute(array(':accountID' => $this->_accountID, ':roomID' => $roomID, ':screenName' => $screenName, ':active' => $this->_active))
        ) {
